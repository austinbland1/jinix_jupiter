...
The Milestone 6D aggregate command is:
...
### Milestone 6D-3 Integrated Multiword DMA Copy

Run the focused end-to-end regression with:

    make -C sim dma-copy-test

The complete Milestone 6D aggregate is:

    make -C sim m6d-test

Milestone 6D-3 verifies a real four-word copy using the synthesizable
`jupiter_dma` engine through the production memory architecture.

A real Jupiter CPU program:

- constructs the external-SDRAM source and destination addresses;
- writes DMA SRC_BASE;
- writes DMA DST_BASE;
- writes LENGTH_WORDS = 4;
- issues CONTROL.START;
- polls STATUS through CPU-visible DMA MMIO until DONE is observed; and
- halts only after the transfer completes.

The CPU program performs no external-SDRAM data accesses itself. Therefore
all external-memory READ and WRITE commands in the focused copy interval are
attributable to the production DMA engine.

The production path under test is:

    Jupiter CPU -> DMA MMIO
                   |
                   v
              jupiter_dma
                   |
...
The behavioral SDRAM model is preloaded with four distinct source words,
four destination sentinels, and unrelated low/high guard words.

The regression verifies:

- exactly four logical DMA source reads;
- exactly four logical DMA destination writes;
- exact source address progression;
- exact destination address progression;
- exact copied 32-bit data and full write strobes;
- source-region preservation;
- exact destination-region contents;
- preservation of both unrelated-memory guards;
- CPU-visible configuration and DONE polling;
- no CPU external-SDRAM transaction;
- no GPU external-SDRAM transaction;
- eight physical 16-bit READ commands;
- eight physical 16-bit WRITE commands;
- sixteen ACTIVE commands;
- no behavioral-SDRAM protocol error; and
- idle production memory interfaces after completion.

The test does not override the production DMA memory-master signals. Memory
traffic is generated by the synthesizable DMA state machine introduced in
M6D-1 and multiword-verified in M6D-2.

Milestone 6D-3 is therefore the first end-to-end verification checkpoint for
a synthesizable multiword external-SDRAM-to-external-SDRAM DMA copy.

Simulation cycle counts and command counts are verification observations
only. They are not measured DMA bandwidth, timing, or performance claims.
