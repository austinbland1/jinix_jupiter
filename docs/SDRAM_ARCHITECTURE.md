...
The current Jupiter `clk_sys` is generated by `rtl/pll.v` from the 50 MHz
`CLK_50M` input and is configured as 20 MHz.

This 20 MHz value is a verified fact about the current template configuration,
not yet a requirement that the final physical SDRAM interface must use the
same clock or phase relationship.
